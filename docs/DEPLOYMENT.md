...
## مینی‌اپِ تلگرام (ADR-0027)

بات `@monad_house_bot` مینی‌اپِ مناد را داخلِ خودِ تلگرام باز می‌کند. کد کامل است؛ فقط دو تنظیم می‌ماند:

1. **توکن در env:** `TELEGRAM_BOT_TOKEN` (از BotFather) را در env تولید بگذارید. بدونِ آن، `/api/tma/auth` با ۵۰۳ خاموش می‌ماند و مینی‌اپ کار نمی‌کند.
2. **دکمه‌ی منو در BotFather:** در گفتگو با `@BotFather` → `/mybots` → انتخابِ بات → **Bot Settings → Menu Button → Configure menu button** → URL را `https://asraar.ir/tma` بگذارید. (یا فرمانِ `/setmenubutton`.)

نکته‌ها:
- **بدونِ گلوگاهِ فیلترینگ:** حلقه‌ی گفتگوی مینی‌اپ فقط با API خودِ مناد حرف می‌زند و اعتبارسنجیِ `initData` آفلاین (HMAC) است؛ هیچ تماسی با `api.telegram.org` ندارد، پس روی سرورِ ایران هم بی‌مانع کار می‌کند.
- **قاب‌شدن (iframe):** مسیرِ `/tma` عمداً از `X-Frame-Options: DENY` مستثناست و با `frame-ancestors` فقط اجازه‌ی قاب‌شدن توسطِ تلگرام را دارد (برای تلگرام دسکتاپ/وب لازم است). بقیه‌ی سایت همچنان کاملاً ضدِ clickjacking است. اگر جلوی `/tma` یک CDN/پراکسی هست، مطمئن شوید این هدرها را بازنویسی/حذف نکند.
- **HTTPS الزامی است:** تلگرام مینی‌اپ را فقط از URLِ HTTPS باز می‌کند — به همین دلیل به استقرارِ آروان + دامنه گره خورده.
- در `/admin/health` ردیفِ «مینی‌اپِ تلگرام» وضعیتِ تنظیمِ توکن را نشان می‌دهد.
